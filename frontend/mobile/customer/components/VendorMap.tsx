...
import { categories, Vendor } from "../constants/vendor";
...
const VendorMap = forwardRef<VendorMapHandle, VendorMapProps>(function VendorMap(
    ...
    useEffect(() => {
        const payload = vendors.map((v) => ({
            ...
            title: v.name,
            color: colorForCategory(v.category),
        }));
        run(`window.setVendors(${JSON.stringify(payload)})`);
    ...
    useEffect(() => {
        if (userLocation && routeTo) {
            run(
                `window.showRoute(${userLocation.latitude}, ${userLocation.longitude}, ${routeTo.latitude}, ${routeTo.longitude})`,
            );
        } else {
